...
-   `level`: the type of information in the log. Your options are `debug`, `info`, `notice`, `warn`, and `error`. The default for the `console` destination is `warn`, for the `file` destination the default is `notice`.
