...
describe 'commands' do
  ...
  describe 'inventory show' do
    context 'with an invalid inventory file' do
    ...
    context 'without a targeting option' do
      let(:inventory) { { 'targets' => %w[foo bar baz] } }

      it 'defaults to showing all targets' do
        result = run_cli_json(%w[inventory show], project: project)
        expect(result['targets']).to match_array(%w[foo bar baz])
      end
    end
  end
