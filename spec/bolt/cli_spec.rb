...
describe Bolt::CLI do
  ...
  describe '#parse' do
    ...
    context 'targeting options' do
      it 'accepts a targeting option' do
        options = Bolt::CLI.new(%w[command run whoami -t localhost]).parse
        expect(options).to include(targets: ['localhost'])
      end

      it 'errors with multiple targeting options' do
        expect {
          Bolt::CLI.new(%w[command run whoami -t localhost --rerun all]).parse
        }.to raise_error(
          Bolt::CLIError,
          /Only one targeting option can be specified/
        )
      end

      it 'errors when missing required targeting option' do
        expect {
          Bolt::CLI.new(%w[command run whoami]).parse
        }.to raise_error(
          Bolt::CLIError,
    ...
    context 'file upload' do
      ...
      it 'errors with env vars' do
        expect {
          Bolt::CLI.new(%w[file upload source destination --targets localhost --env-var foo=bar]).parse
        }.to raise_error(
          Bolt::CLIError,
    ...
    context '--clear-cache' do
      ...
      before(:each) do
        ...
        allow(project).to receive_messages(
          plan_cache_file:   plan_cache,
          plugin_cache_file: plugin_cache,
          task_cache_file:   task_cache
        )
      end
      ...
      it 'clears cache for plug-in, plan, and task cache' do
        ...
        expect(FileUtils).to receive(:rm).with(plugin_cache)
        expect(FileUtils).to receive(:rm).with(plan_cache)
        expect(FileUtils).to receive(:rm).with(task_cache)

        cli.execute(clear_cache: true)
      end
    end
  end
  ...
  describe '#process_command' do
    ...
    context 'command run' do
      before(:each) do
        expect(application).to receive(:run_command).and_return(result)
      end
    ...
    context 'file download' do
      before(:each) do
        expect(application).to receive(:download_file).and_return(result)
      end
    ...
    context 'file upload' do
      before(:each) do
        expect(application).to receive(:upload_file).and_return(result)
      end
    ...
    context 'group show' do
      before(:each) do
        expect(application).to receive(:list_groups).and_return({})
      end
    ...
    describe 'guide' do
      before(:each) do
        expect(application).to receive(:list_guides).and_return({})
      end
    ...
    describe 'inventory show' do
      before(:each) do
        expect(application).to receive(:show_inventory).and_return(result)
      end
    ...
    context 'module add' do
      before(:each) do
        expect(application).to receive(:add_module)
      end
      ...
      it 'returns FAILURE with a failing result' do
        allow(application).to receive(:add_module).and_return(false)
        expect(cli.execute(options)).to eq(Bolt::CLI::FAILURE)
      end
    end
    ...
    context 'module generate-types' do
      before(:each) do
        expect(application).to receive(:generate_types)
      end
    ...
    context 'module install' do
      before(:each) do
        expect(application).to receive(:install_modules)
      end
      ...
      it 'returns FAILURE with a failing result' do
        allow(application).to receive(:install_modules).and_return(false)
        expect(cli.execute(options)).to eq(Bolt::CLI::FAILURE)
      end
    end
    ...
    context 'module show' do
      before(:each) do
        expect(application).to receive(:list_modules).and_return(result)
      end
    ...
    context 'plan convert' do
      before(:each) do
        expect(application).to receive(:convert_plan)
      end
    ...
    context 'plan new' do
      before(:each) do
        expect(application).to receive(:new_plan).and_return({})
      end
    ...
    context 'plan run' do
      before(:each) do
        expect(application).to receive(:run_plan).and_return(result)
      end
    ...
    context 'plugin show' do
      before(:each) do
        expect(application).to receive(:list_plugins).and_return({})
      end
    ...
    context 'project init' do
      before(:each) do
        expect(application).to receive(:create_project)
      end
    ...
    context 'project migrate' do
      before(:each) do
        expect(application).to receive(:migrate_project)
      end
    ...
    context 'script run' do
      before(:each) do
        expect(application).to receive(:run_script).and_return(result)
      end
    ...
    context 'secret createkeys' do
      before(:each) do
        expect(application).to receive(:create_secret_keys).and_return(result)
      end
    ...
    context 'secret decrypt' do
      before(:each) do
        expect(application).to receive(:decrypt_secret).and_return(result)
      end
    ...
    context 'secret encrypt' do
      before(:each) do
        expect(application).to receive(:encrypt_secret).and_return(result)
      end
    ...
    context 'task run' do
      before(:each) do
        expect(application).to receive(:run_task).and_return(result)
      end
