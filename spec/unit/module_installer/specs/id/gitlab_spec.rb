...
require 'bolt/module_installer/specs/id/gitlab'

describe Bolt::ModuleInstaller::Specs::ID::GitLab do
  let(:id)       { described_class.request(git, ref, nil) }
  let(:git)      { "https://gitlab.com/puppetlabs/#{name}" }
  let(:ref)      { '0.1.0' }
  ...
  before(:each) do
    allow(described_class).to receive(:make_request)
      .with(%r{gitlab.com/puppetlabs}, any_args)
      .and_return(metadata_response)
    allow(described_class).to receive(:make_request)
      .with(%r{gitlab.com/api}, any_args)
  ...
  it 'returns nil if not a GitLab repo' do
    id = described_class.request('https://github.com/puppetlabs/puppetlabs-yaml', ref, nil)
    expect(id).to eq(nil)
  end
