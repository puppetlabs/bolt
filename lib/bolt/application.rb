...
module Bolt
  class Application
    ...
    def file_upload(source, destination, targets)
      ...
      Bolt::Util.validate_file('source file', source)
    ...
    # TODO: Should this take an outputter or return data that the CLI
    #       prints with the outputter?
    #
    # Show Bolt guides.
    #
    # @param topic [String, NilClass] The topic to show.
    # @param outputter [Bolt::Outputter] An outputter instance.
    # @return [Boolean]
    #
    def guide(topic, outputter)
      guides = load_guides

      if topic
        show_guide(topic, guides, outputter)
      else
        list_guides(guides, outputter)
      end
    end
    ...
    # @return [Boolean]
    #
    private def list_guides(guides, outputter)
    ...
    # @return [Boolean]
    #
    private def show_guide(topic, guides, outputter)
      if guides[topic]
    ...
    #
    # @param targets [Array[String], NilClass] The targets to show.
    # @return [Hash]
    #
    def inventory_show(targets)
    ...
    # TODO: This should probably be separate #lookup and #plan_lookup methods.
    #
    ...
    # @param plan_hierarchy [Boolean] Lookup values using Hiera's plan_hierarchy.
    # @param vars [Hash] Variables to set in the scope.
    # @return [Bolt::ResultSet, String] The result of the lookup.
    #
    def lookup(key, targets, plan_hierarchy: false, vars: {})
      if plan_hierarchy
    ...
    def module_install(outputter, force: false, resolve: true)
      ...
      if config.project.modules.empty? && force
    ...
    # TODO: This should probably be separate #show_plan and #list_plans methods.
    #
    # Show plan information.
    #
    # @param plan [String, NilClass] The name of the plan to show.
    # @param filter [String, NilClass] A string to filter plans by.
    # @return [Hash]
    #
    def plan_show(plan, filter: nil)
      if plan
        pal.get_plan_info(plan)
      else
        plans = filter_content(pal.list_plans_with_cache(filter_content: true), filter)
        { plans: plans, modulepath: pal.user_modulepath }
      end
    end
    ...
    # TODO: This should probably be separate #show_task and #list_tasks methods.
    #
    # Show task information.
    #
    # @param task [String, NilClass] The name of the task to show.
    # @param filter [String, NilClass] A string to filter tasks by.
    # @return [Boolean]
    #
    def task_show(task, filter: nil)
      if task
        pal.get_task(task)
      else
        tasks = filter_content(pal.list_tasks(filter_content: true), filter)
        { tasks: tasks, modulepath: pal.user_modulepath }
      end
    end
    ...
    # TODO: Could this go in Bolt::Inventory? Possibly.
    #
    ...
    private def with_benchmark
      ...
      result.tap { |r| r.elapsed_time = elapsed_time }
    end
  end
end
