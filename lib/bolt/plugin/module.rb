  ...
  class Plugin
    class Module
      ...
      def setup
        ...
        # Raise an error if there is config in the plugin data
        if @data['config']
          msg = <<~MSG
            'config' key is not allowed in 'bolt_plugin.json'. Plugin config is inferred from
            task parameters.
          MSG
          raise InvalidPluginData.new(name, msg)
        end
