  ...
  class Config
    class SSH < Sudoable
      ...
      OPTIONS = {
        ...
        "host"               => { type: String,
                                  desc: "Host name." },
        "host-key-check"     => { desc: "Whether to perform host key validation when connecting." },
        ...
        "load-config"        => { desc: "Whether to load system SSH configuration." },
        ...
        "tty"                => { desc: "Request a pseudo tty for the session. This option is generally "\
      ...
      private def validate
        ...
        assert_boolean('host-key-check', 'load-config', 'tty')
