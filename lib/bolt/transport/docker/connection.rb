    ...
    class Docker < Base
      class Connection
        ...
        def execute(*command, options)
          ...
          result = execute_local_docker_command('exec', command_options, options[:stdin])

          # The actual result is the exitstatus not the process object
          result[2] = result[2].nil? ? -32768 : result[2].exitstatus
          if result[2] == 0
            @logger.debug { "Command returned successfully" }
          else
            @logger.info { "Command failed with exit code #{result[2]}" }
          end
          result[0] = result[0].force_encoding('UTF-8')
          result[1] = result[1].force_encoding('UTF-8')
