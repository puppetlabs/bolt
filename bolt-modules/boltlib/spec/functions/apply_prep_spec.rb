...
describe 'apply_prep' do
  ...
  context 'with targets' do
    ...
    it 'sets feature and gathers facts' do
      versions = Bolt::ResultSet.new(targets.map { |t| Bolt::Result.new(t, message: '5.0.0') })
      Puppet::Pops::Functions::Function.any_instance.expects(:call_function).with(
        :run_task, 'puppet_agent::version', targets
      ).returns(versions)
    ...
    it 'installs the agent if not present' do
      versions = Bolt::ResultSet.new(
        [Bolt::Result.new(targets[0], message: '5.0.0'), Bolt::Result.new(targets[1], message: '')]
      )
      Puppet::Pops::Functions::Function.any_instance.expects(:call_function).with(
        :run_task, 'puppet_agent::version', targets
      ).returns(versions)
      ...
      Puppet::Pops::Functions::Function.any_instance.expects(:call_function).with(
        :run_task, 'puppet_agent::install', targets[1..1]
      ).returns(versions)
    ...
    it 'fails if version check fails' do
      task = mock('version_task', runnable_with?: true, task: :version_task)
      Puppet::Pal::ScriptCompiler.any_instance.expects(:task_signature).with('puppet_agent::version').returns(task)
    ...
    it 'fails if install fails' do
      task1 = mock('version_task', runnable_with?: true, task: :version_task)
      Puppet::Pal::ScriptCompiler.any_instance.expects(:task_signature).with('puppet_agent::version').returns(task1)

      versions = Bolt::ResultSet.new(targets.map { |t| Bolt::Result.new(t, message: '') })
      executor.expects(:run_task).with(targets, :version_task, anything, anything).returns(versions)

      task2 = mock('install_task', runnable_with?: true, task: :install_task)
      Puppet::Pal::ScriptCompiler.any_instance.expects(:task_signature).with('puppet_agent::install').returns(task2)
