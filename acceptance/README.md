...
You may also need to specify nodes for [beaker-hostgenerator]() to use
to test bolt on. You can see all of the available
...
**BOLT_CONTOLLER** (Default `centos7-64bolt,ssh.`): Operating system
and architecture to be used for the node running `bolt`. This value
should be expressed in the notation used by
[beaker-hostgenerator](https://github.com/puppetlabs/beaker-hostgenerator)
(i.e.`windows10ent-64bolt,winrm`), with the 'bolt' role appended as
well as either 'ssh' or 'win'. See the [beaker-hostgenerator
usage](https://github.com/puppetlabs/beaker-hostgenerator#usage) for
more information about formatting, and see the [examples](#examples)
below for examples. 

**BOLT_NODES** (Default
`ubuntu1604-64ssh.-osx1012-64ssh.-windows10ent-64winrm.`): Operating
system and architecture to be used for the nodes targeted by the bolt
tests. This value should be expressed in
the notation used by
[beaker-hostgenerator](https://github.com/puppetlabs/beaker-hostgenerator#usage).
Multiples should be hyphen separated. See [examples](#examples) below
for examples.
...
be checked out for testing.  This variable supercedes `GIT_BRANCH` if
provided.
...
node
```
BOLT_CONTROLLER='ubuntu1604-64bolt,ssh.' \
BOLT_NODES='redhat7-64ssh.' \
